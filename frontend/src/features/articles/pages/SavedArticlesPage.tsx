...
import { getSavedArticles } from '@/api/users'
...
export function SavedArticlesPage() {
  ...
  const userInteractions = useMemo(() => getUserInteractions(currentUser.id), [currentUser.id, refreshKey])
  ...
  const commentedArticleIds = useMemo(() => {
    return [...new Set(userInteractions.filter((i) => i.type === 'comment').map((i) => i.articleId))]
      ...
      <div className="container mx-auto">
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-3">
          <div className="rounded-lg border bg-card p-6">
            ...
            <p className="text-sm text-muted-foreground">保存した記事</p>
          </div>
          <div className="rounded-lg border bg-card p-6">
            <div className="mb-2 flex items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground">コメントした記事</p>
          </div>
          <div className="rounded-lg border bg-card p-6">
            <div className="mb-2 flex items-center gap-2">
              <Users className="h-5 w-5 text-primary" />
              <span className="text-2xl font-bold">{participatedSessionsCount}</span>
            </div>
            <p className="text-sm text-muted-foreground">参加したセッション</p>
          </div>
        </div>
